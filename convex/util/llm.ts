...
export interface LLMConfig {
  provider: 'openai' | 'together' | 'ollama' | 'custom' | 'deepseek';
...
// Pick one key from a comma-separated pool (the "API 池" from our blueprint).
// Random per-call rotation spreads load / rate limits across all keys.
function pickPooledKey(raw: string | undefined): string | undefined {
...
export function getLLMConfig(): LLMConfig {
  ...
  // DeepSeek: OpenAI-compatible chat at /v1/chat/completions. It has NO embedding
  // or moderation endpoints, so embeddings fall back to a local deterministic
  // vector (see fetchEmbeddingBatch). Keys come from a pool via DEEPSEEK_API_KEYS
  // (comma-separated) and rotate per call.
  if (
    provider === 'deepseek' ||
    (!provider && (process.env.DEEPSEEK_API_KEYS || process.env.DEEPSEEK_API_KEY))
  ) {
    return {
      provider: 'deepseek',
      ...
      stopWords: [],
      apiKey: pickPooledKey(process.env.DEEPSEEK_API_KEYS ?? process.env.DEEPSEEK_API_KEY),
    };
  }
...
export async function fetchEmbeddingBatch(texts: string[]) {
  ...
  if (config.provider === 'deepseek' || config.embeddingModel === 'local') {
    // DeepSeek has no embedding endpoint. Use a deterministic local hash
    // embedding so the memory vector index keeps working without a second
    // provider. Phase-1 stopgap: lexical signal only — swap in a real
    // embedding provider when memory-recall quality starts to matter.
    return {
      ollama: false as const,
      embeddings: texts.map((t) => localHashEmbedding(t)),
      usage: 0,
      retries: 0,
      ms: 0,
    };
...
// Deterministic, network-free embedding used when the provider has no embedding
// endpoint (e.g. DeepSeek). Hashes tokens (incl. CJK runs) into a fixed-size
// vector and L2-normalizes. Carries lexical signal only; not semantic.
